...
function bindSearch(request: sql.Request, search: string): { where: string } {
  ...
  return { where: "WHERE (p.PermissionKey LIKE @q OR p.Description LIKE @q)" };
}

/** List with paging + search; includes AssignedCount from RolePermissions */
export async function listPermissions(params: ListPermissionsParams): Promise<ListPermissionsResult> {
  ...
  const dataResult = await reqData.query<PermissionRowSQL>(`
    SELECT p.PermissionID,
           p.PermissionKey,
           p.Description,
           p.CreatedAt,
           p.UpdatedAt,
           (SELECT COUNT(*) FROM dbo.RolePermissions rp WHERE rp.PermissionID = p.PermissionID) AS AssignedCount
  ...
  const countResult = await reqCount.query<CountRow>(`
    SELECT COUNT(1) AS Total FROM dbo.Permissions p ${where};
  `);
...
export async function getPermissionById(id: number): Promise<PermissionDTO | null> {
  ...
  const r = await p.request()
    ...
    .query<PermissionRowSQL>(`
      SELECT p.PermissionID, p.PermissionKey, p.Description, p.CreatedAt, p.UpdatedAt,
             (SELECT COUNT(*) FROM dbo.RolePermissions rp WHERE rp.PermissionID = p.PermissionID) AS AssignedCount
      FROM dbo.Permissions p
...
/** Create — returns new PermissionID */
export async function createPermission(input: CreatePermissionInput): Promise<number> {
  try {
    ...
    const r = await p.request()
      ...
      `);
    return r.recordset[0].PermissionID;
  } catch (err: unknown) {
...
export async function updatePermission(id: number, input: UpdatePermissionInput): Promise<boolean> {
  try {
    const p = await poolPromise;
    const r = await p.request()
      .input("id", sql.Int, id)
