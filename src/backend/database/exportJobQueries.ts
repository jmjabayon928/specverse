...
/** Atomically claim a job (queued or reclaimable). Uses UPDLOCK/ROWLOCK. Returns claimed and row. */
export async function claimExportJob(
  ...
  const result = await pool
    ...
    .query(`
      ...
      FROM dbo.ExportJobs ej WITH (UPDLOCK, ROWLOCK)
      WHERE ej.Id = @Id
        AND (ej.Status = 'queued' OR (ej.Status = 'running' AND (ej.LeasedUntil IS NULL OR ej.LeasedUntil < @Now)))
    `)
  const row = result.recordset?.[0] as ExportJobRow | undefined
...
export async function updateExportJobFailed(
  ...
  await req.query(`
      ...
      SET Status = 'failed', CompletedAt = @CompletedAt, ExpiresAt = @ExpiresAt,
          ErrorMessage = @ErrorMessage, LeaseId = NULL, LeasedUntil = NULL
      ${whereClause}
    `)
}
