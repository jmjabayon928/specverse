          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.RoleID} className="border-b">
                ...
                <td className="py-2">{r.RoleDesc ?? "—"}</td>
                <td className="py-2">{r.UsersCount ?? 0}</td>
                <td className="py-2">
                  <button
...
function RoleForm(props: Readonly<{
  ...
  const [RoleDesc, setRoleDesc] = useState(initial?.RoleDesc ?? "");
  ...
  const submit = async () => {
    ...
    const payload = { RoleName: RoleName.trim(), RoleDesc: (RoleDesc ?? "").trim() || null };
