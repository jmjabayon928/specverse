...
// Option B central definitions with a shared normalizeUnit(), no imports (to avoid cycles).
...
export type QuantityKind =
  ...
  | "thermal_resistance"
  | "specific_heat"
  | "energy_per_mass_at_temp" // kJ/kg @ °C, BTU/lb @ °F (treated as same kind for UI)
  | "force_per_area" // fallback bucket for N/m², kg/m·s² (rare edge formatting);

// Canonical normalizer used everywhere (UI inputs, table keys, lookups, etc.)
export function normalizeUnit(raw: string): string {
  if (!raw) return "";
  // 1) Lowercase for matching & kill whitespace
  let s = raw.trim().toLowerCase();

  // 2) Unicode/symbol cleanup
  s = Array.from(s, (ch) => {
    ...
    if (cp >= 0x2080 && cp <= 0x2089) {
      return String.fromCodePoint(cp - 0x2080 + 0x30);
    }
  ...
  // 4) Notation harmonization for superscripts & separators (prefer ascii)
  //    "m³/h" → "m3/h", "w/m.k" → "w/m.k", "w/m2.k" ok, etc.
  s = s
    .replaceAll("m³", "m3")
    .replaceAll("m²", "m2")
    .replaceAll("ft²", "ft2")
  ...
  // 5) Strip optional parentheses/annotations like "(a)" "(g)" "@c" etc. from *keys*,
  //    because your conversionTable keys are normalized before map indexing.
  //    Keep slash/dot structure since we use it to bucket kinds.
  s = s.replaceAll(/\(.*?\)/g, "");
...
export const UNIT_KIND: Record<string, QuantityKind> = {
  ...
  // temperature (we accept c/f/k as tokens)
  c: "temperature",
  f: "temperature",
  k: "temperature",
  ...
  "btu/(hr.ft2.f)": "heat_transfer_coeff", // (area version—often used as U-value)
  ...
  // energy per mass at temperature (for your "kJ/kg @ °C" case)
  "kj/kg@c": "energy_per_mass_at_temp",
  "btu/lb@f": "energy_per_mass_at_temp",

  // force per area fallback (rare formatting like kg/m.s2 → treat as N/m2)
  "n/m2": "force_per_area",
  "kg/m.s2": "force_per_area",
};

// Same-system alternates (for quick in-system switches in UI)
