...
export type BlockType =
  | "Subsheet" | "Field" | "Table"
  | "Text" | "Image" | "QRCode" | "Signature" | "Spacer"
...
export interface LayoutBlock extends GridRect {
  ...
  sourceRef?: {
    ...
    columns?: Array<{ label?: string; infoTemplateID?: number; expr?: string }>;
    url?: string;
...
export interface RenderField {
  infoTemplateId: number;
  label: string;         // translated already
  value: string;         // formatted per UOM
...
export interface RenderBlock {
  blockId?: number;      // present if from LayoutBlocks; absent for body-only slots
  title?: string;
  fields?: RenderField[];
  subsheetId?: number;
