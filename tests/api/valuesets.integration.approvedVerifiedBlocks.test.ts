/**
 * Phase 2 integration: Approved/Verified sheet — POST valueset returns 409, no value set created, bump not called.
 * Real route + controller + valueSetService (not invoked); mock only db, sheetAccessService, getFilledSheetDetailsById.
 */
...
describe('Approved/Verified sheet: POST valueset returns 409, no insert, no bump', () => {
  ...
  it('POST valueset when status is Approved returns 409 and no INSERT into InformationValueSets', async () => {
    mockGetFilledSheetDetailsById.mockResolvedValue({
      datasheet: { status: 'Approved', sheetId: testSheetId },
    })
    ...
    expect(mockBumpRejectedToModifiedDraftFilled).not.toHaveBeenCalled()
    const insertValueSetQueries = queryLog.filter(
      q => q.includes('INSERT') && q.includes('InformationValueSets')
    )
    expect(insertValueSetQueries.length).toBe(0)
  })

  it('POST valueset when status is Verified returns 409 and no INSERT into InformationValueSets', async () => {
    mockGetFilledSheetDetailsById.mockResolvedValue({
      datasheet: { status: 'Verified', sheetId: testSheetId },
    })
